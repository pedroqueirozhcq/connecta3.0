...
function CoordinatorDashboardContent() {
  ...
  const { data: leaders } = useCollection(teamLeadersQuery);
  ...
  const simulatedLeaders = [
    { id: "s1", name: "Ricardo Silva", email: "ricardo@connecta.com", progress: 42, status: "ATIVO", missionsDone: 15, totalMissions: 20 },
    { id: "s2", name: "Juliana Mendes", email: "juliana@connecta.com", progress: 58, status: "ATIVO", missionsDone: 22, totalMissions: 25 },
    { id: "s3", name: "Marcos Vinícius", email: "marcos@connecta.com", progress: 35, status: "ATIVO", missionsDone: 10, totalMissions: 18 },
    { id: "s4", name: "Ana Paula", email: "ana@connecta.com", progress: 67, status: "ATIVO", missionsDone: 30, totalMissions: 35 },
  ];

  const filteredLeaders = useMemo(() => {
    return simulatedLeaders.filter(l => 
      l.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      l.email.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [searchQuery]);

  const [teamDeliveries, setTeamDeliveries] = useState([
  ...
  const handleSendMessage = (leaderId: string, leaderName: string) => {
    ...
    toast({
      ...
      description: `As orientações foram transmitidas para ${leaderName}.`,
    });
    setMessage("");
  };
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 animate-in fade-in duration-500">
      <div className="border-b border-white/5 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-headline text-white tracking-tight">Painel <span className="text-primary">Tático</span></h1>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                  <div className="flex items-center gap-4 flex-1">
                    <Avatar className="h-11 w-11 rounded-lg ring-1 ring-accent/20">
                      <AvatarFallback className="bg-accent/10 text-accent font-bold text-xs">{leader.name[0]}</AvatarFallback>
                    </Avatar>
                    <div className="space-y-1">
                      ...
                      <Badge className={`${leader.status === 'ATIVO' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'} border-none text-[9px] px-2 h-4 font-black uppercase tracking-widest mt-1`}>
                        {leader.status}
                      </Badge>
                    </div>
                  </div>

                  <div className="flex-1 space-y-2 px-6 md:border-l border-white/5">
                    <div className="flex justify-between text-[10px] font-black text-muted-foreground uppercase tracking-widest">
                      <span>Missões Concluídas</span>
                      <span className="text-accent">{Math.round((leader.missionsDone / leader.totalMissions) * 100)}%</span>
                    </div>
                    <Progress value={(leader.missionsDone / leader.totalMissions) * 100} className="h-1.5" indicatorClassName="bg-accent" />
                    <div className="text-[11px] text-muted-foreground mt-1">
                      Finalizadas: <span className="text-white font-bold">{leader.missionsDone}/{leader.totalMissions}</span>
                    </div>
                  </div>
                    ...
                    <DialogContent className="bg-card border-white/10 text-white sm:max-w-[450px]">
                      <DialogHeader>
                        <DialogTitle className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-lg bg-accent/10 flex items-center justify-center text-accent font-bold text-sm">{leader.name[0]}</div>
                          <div className="text-left">
                            <div className="text-lg font-headline">Chat Direto: {leader.name}</div>
                      ...
                      <div className="space-y-4 py-4">
                        <div className="bg-secondary/20 p-5 rounded-xl border border-white/5 min-h-[150px] max-h-[300px] overflow-y-auto flex flex-col gap-4">
                           <div className="bg-accent/10 p-3.5 rounded-2xl rounded-tr-none self-end max-w-[85%] border border-accent/20">
                            <p className="text-sm text-white">Olá {leader.name.split(' ')[0]}, precisamos alinhar os prazos das novas missões.</p>
                            <span className="text-[10px] text-muted-foreground mt-2 block text-right">Agora</span>
                          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {operationalMissions.map(m => {
                const urgencyColor = m.urgency?.includes("1h") 
                  ? "border-red-500" 
                  : m.urgency?.includes("3h") 
                  ? "border-yellow-500" 
                  : "border-green-500";
                
                const badgeColor = m.urgency?.includes("1h") 
                ...
                return (
                  <Card key={m.id} className={`bg-card border-none border-b-2 ${urgencyColor} overflow-hidden group`}>
                    <CardContent className="p-5 space-y-4">
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          <h3 className="text-sm font-bold text-white group-hover:text-primary transition-colors">{m.title}</h3>
                        </div>
                        <Badge variant="outline" className={`${badgeColor} text-[10px] font-black uppercase tracking-widest px-2 h-5`}>
                          {m.urgency}
                        </Badge>
                      </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="flex gap-3 w-full md:auto">
                <div className="relative flex-1 md:w-48">
                  ...
                  <Input 
                    placeholder="Filtrar Missão..." 
                    className="bg-card border-white/5 pl-10 h-10 text-xs focus:ring-accent"
                ...
                <Card key={delivery.id} className={`bg-secondary/5 border-white/5 border-l-4 transition-all ${delivery.status === 'Avaliada' ? 'border-green-500 opacity-60' : 'border-accent'}`}>
                  <CardContent className="p-6 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="flex-1 space-y-3">
                      ...
                      <p className="text-xs text-muted-foreground italic line-clamp-1">"Evidências sincronizadas no repositório. Aguardando validação tática."</p>
                    </div>

                    <div className="flex flex-col md:flex-row items-center gap-8">
                      <div className="flex flex-col items-center md:items-end gap-2">
                        <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Critério de Performance</span>
                        <StarRating 
        ...
        <TabsContent value="ranking" className="space-y-6 m-0 outline-none">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              ...
              <p className="text-muted-foreground text-[11px] uppercase font-bold tracking-widest mt-1">Classificação Tática por Missões Realizadas</p>
            </div>
          ...
          {rankingCategory === "Leader" && (
            <div className="flex gap-3 border-b border-white/5 pb-5">
              {["Todos", "Equipe da Comunicação", "Equipe do Externo"].map(team => (
                <Button 
                  key={team}
                  variant={rankingTeam === team ? "outline" : "ghost"}
                  onClick={() => setRankingTeam(team)}
                  size="sm"
                  className={`h-8 text-[10px] font-bold uppercase tracking-widest ${rankingTeam === team ? "border-accent text-accent" : "text-muted-foreground"}`}
                >
                  {team}
                </Button>
              ))}
            ...
            <Table>
              <TableHeader className="bg-secondary/30">
                <TableRow className="border-white/5">
                  ...
                  <TableHead className="text-right px-10">Performance</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                  ...
                  <TableRow key={item.id} className="border-white/5 hover:bg-white/5 transition-colors group">
                    <TableCell className="text-center">
                      <div className="flex flex-col items-center">
                        <span className={`text-base font-black ${index === 0 ? "text-yellow-500" : index === 1 ? "text-slate-400" : index === 2 ? "text-amber-700" : "text-muted-foreground"}`}>
                          #{index + 1}
                        </span>
                        {index === 0 && <Trophy className="w-4 h-4 text-yellow-500 mt-1" />}
                        {index === 1 && <Medal className="w-4 h-4 text-slate-400 mt-1" />}
                        {index === 2 && <Award className="w-4 h-4 text-amber-700 mt-1" />}
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-4">
                        <div className="relative">
                          <Avatar className="h-12 w-12 border border-white/10 group-hover:border-accent/50 transition-colors">
                            <AvatarFallback className="bg-accent/10 text-accent font-black text-sm">{item.name[0]}</AvatarFallback>
                          </Avatar>
                          <div className="absolute -bottom-1 -right-1 bg-background rounded-full p-1 shadow-sm">
                            {item.role === "Coordinator" ? <ShieldCheck className="w-4 h-4 text-primary" /> : <Star className="w-4 h-4 text-accent fill-accent" />}
                          </div>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-white font-bold text-base">{item.name}</span>
                          <span className="text-[10px] text-muted-foreground uppercase font-black tracking-widest">{item.role === 'Coordinator' ? 'Coordenador Master' : 'Líder Operacional'}</span>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge variant="secondary" className="bg-primary/10 text-primary border-primary/20 text-xs font-black uppercase tracking-widest">
                        {item.team}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right px-10">
                      <div className="flex flex-col items-end">
                        <span className="text-accent font-black text-sm uppercase tracking-widest">{item.missions} MISSÕES</span>
                        <span className="text-[10px] text-muted-foreground font-bold uppercase tracking-tighter">Realizadas</span>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
                {rankingData.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={4} className="text-center py-16 text-muted-foreground italic text-base">
                      Nenhum operador encontrado nesta categoria.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
...
export default function CoordinatorDashboard() {
  return (
    <Suspense fallback={<div className="p-6 text-white animate-pulse">Sincronizando Painel Tático...</div>}>
